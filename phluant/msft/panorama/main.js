...
function prepareEvent(){
	...
	$('#minimize').click(function(){
		...
		if(firstTimeout){
			firstTimeout = false;
			setTimeout(function(){
				transform2d('full_image',1,0,-100,0,false,false,0.2,'ease-in-out');
				setTimeout(function(){
					transform2d('full_image',1,0,0,0,false,false,0.2,'ease-in-out');
				},1000);
				
				setTimeout(function(){
					if(enableAutoScroll){
						if($(window).width() < 1600){
							$('#full').animate({scrollLeft: 1600 - $(window).width() }, 750);
						}
					}
				}, 6000);
			},1250);
