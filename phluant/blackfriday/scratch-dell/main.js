...
function prepareAnimation(){
    ...
    enableTransform2d('slide',1,0,0,0,0.5,'ease-in-out');
...
function scratch(){
    ...
    obj.bind('touchmove', function(e){
        ...
        var leftX = endX - startX;
        $('#scratch').css('-webkit-transform', 'translate3d('+leftX+'px, 0px, 0px)');
        
        if(parseFloat(Math.abs(leftX))/widthScratch >= 0.3){
        	endScratch = true;
        	enableTransform2d('scratch',1,0,0,0,0.5,'linear');
        	setTimeout(function(){
        		if(leftX > 0){
            		$('#scratch').css('-webkit-transform', 'translate3d('+widthScratch+'px, 0px, 0px)');
            	}
            	else{
            		$('#scratch').css('-webkit-transform', 'translate3d(-'+widthScratch+'px, 0px, 0px)');
            	}
    ...
    obj.bind('touchend', function(e){
    	if(!endScratch){
    		$('#scratch').css('-webkit-transform', 'translate3d(0px, 0px, 0px)');
    	}
    });
}
	...
	loadInfo: function(url, type){
        $.get(url, function(d){
            ...
            TerraAds.contentString = '<div id="content_infor">' +
                '<img class="map-image" src="img/inspiron.jpg"/>'+
