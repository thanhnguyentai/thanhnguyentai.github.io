PhluantAds = {
	...
	drawWalk: function(){
		...
		if(this.config.x < 10){
			var timeout = setTimeout(function(){
				...
				self.ctx.drawImage(self.walk, self.walk_frame*self.config.width, 0, self.config.width, self.config.height, self.config.x, self.config.y, self.config.width, self.config.height);
				self.ctx.stroke();
	...
	drawSwing: function(){
		...
		setTimeout(function(){
			self.ctx.clearRect(0,0,self.canvas.width,self.canvas.height);
			self.ctx.drawImage(self.swing, self.swing_frame*self.config.width, 0, self.config.width, self.config.height, self.config.x, self.config.y, self.config.width, self.config.height);
			self.ctx.stroke();
	...
	drawWalkSwing: function(){
		...
		setTimeout(function(){
			self.ctx.clearRect(0,0,self.canvas.width,self.canvas.height);
			self.ctx.drawImage(self.walk_swing, self.walk_swing_frame*self.config.width, 0, self.config.width, self.config.height, self.config.x, self.config.y, self.config.width, self.config.height);
			self.ctx.stroke();
	...
	drawSwingRotate: function(){
		...
		setTimeout(function(){
			self.ctx.clearRect(0,0,self.canvas.width,self.canvas.height);
			self.ctx.drawImage(self.swing_rotate, self.swing_rotate_frame*self.config.width, 0, self.config.width, self.config.height, self.config.x, self.config.y, self.config.width, self.config.height);
			self.ctx.stroke();
	...
	drawWalkInvert: function(){
		var self = this;
		if(this.config.x <= -210){
			setTimeout(function(){
				self.ctx.clearRect(0,0,self.canvas.width,self.canvas.height);
				self.ctx.save();
				self.ctx.scale(-1, 1);
				self.ctx.drawImage(self.walk, self.walk_frame*self.config.width, 0, self.config.width, self.config.height, self.config.x, self.config.y+68, self.config.newWidth, self.config.newHeight);
				self.ctx.stroke();
				self.ctx.restore();
	...
	drawRoar: function(){
		...
		setTimeout(function(){
			...
			self.ctx.save();
			self.ctx.scale(-1, 1);	
		self.ctx.drawImage(self.roar, self.roar_frame*self.config.width, 0, self.config.width, self.config.height, self.config.x, self.config.y+68, self.config.width/3, self.config.height/3);
			self.ctx.stroke();
			self.ctx.restore();
	...
	frameText1In: function(){
		...
		AnimationBase.transform2d('frame1_text4',1,0,0,10,true,false,0.5,'linear');
		AnimationBase.transform2d('frame1_text5',1,0,0,10,true,false,0.5,'linear');
	...
	frameText1Out: function(){
		...
		setTimeout(function(){
			AnimationBase.transform2d('frame1_text4',1,0,0,25,false,true,0.5,'linear');
		}, 750);
		setTimeout(function(){
			AnimationBase.transform2d('frame1_text5',1,0,0,25,false,true,0.5,'linear');
		}, 1000);
	...
	frameText2In: function(){
		...
		var self = this;
		setTimeout(function(){
			AnimationBase.transform2d('button',1,0,148,0,false,false,0.5,'linear');
			self.config.x = -350;
			setTimeout(function(){
				self.drawWalkInvert();
			}, 1000);
